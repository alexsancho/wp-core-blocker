 ...
 * Plugin URI: https://github.com/alexsancho/wp-core-blocker
...
if ( ! class_exists( __NAMESPACE__ . '\Core_Blocker' ) ) {
	...
	class Core_Blocker {
		...
		public static function init() {
			...
			add_action( 'admin_init', [ __CLASS__, 'remove_update_crons' ] );
			...
			add_filter( 'map_meta_cap', [ __CLASS__, 'prevent_auto_updates' ], 10, 2 );

			// Remove bulk action for updating themes/plugins.
			add_filter( 'bulk_actions-plugins', [ __CLASS__, 'remove_bulk_actions' ] );
			...
			add_action( 'network_admin_menu', [ __CLASS__, 'ms_admin_menu_items' ], 9999 );
			add_filter( 'install_plugins_tabs', [ __CLASS__, 'plugin_add_tabs' ] );

			// Theme update API for different calls.
			add_filter( 'themes_api_args', [ __CLASS__, 'bypass_theme_api' ], 10, 2 );
			add_filter( 'themes_api', [ __CLASS__, '__return_false' ], 10, 2 );

			// Time based transient checks.
			add_filter( 'pre_site_transient_update_themes', [ __CLASS__, 'last_checked_themes' ] );
			add_filter( 'pre_site_transient_update_plugins', [ __CLASS__, 'last_checked_plugins' ] );
			add_filter( 'pre_site_transient_update_core', [ __CLASS__, 'last_checked_core' ] );
			add_filter( 'site_transient_update_themes', [ __CLASS__, 'remove_update_array' ] );
			add_filter( 'site_transient_update_plugins', [ __CLASS__, 'remove_plugin_updates' ] );

			// Remove admin news dashboard widget
			add_action( 'admin_init', [ __CLASS__, 'remove_dashboards' ] );
			...
			if ( self::enabled() ) {
				...
				add_filter( 'pre_site_transient_available_translations', [ __CLASS__, 'available_translations' ] );
		...
		/**
		 * Checks when plugin should be enabled This offers nice compatibility with wp-cli
		 */
		 ...
		 */
		public static function remove_dashboards() {
		 ...
		 * @return null
		 */
		public static function ms_admin_menu_items() {
		 ...
		 * @return null
		 */
		public static function remove_update_crons() {
		 ...
		 * @return null
		 */
		public static function remove_schedule_hook() {
